    ...
    public static function fromArray(array $array): self
    {
        ...
        if (isset($array['limit'])) {
            if (!is_numeric($array['limit'])) {
                throw new \InvalidArgumentException(sprintf('Limit must be an number or a numeric string, given: %s', get_debug_type($array['limit'])), 1680785658);
            }
        ...
        if (isset($array['offset'])) {
            if (!is_numeric($array['offset'])) {
                throw new \InvalidArgumentException(sprintf('Offset must be an number or a numeric string, given: %s', get_debug_type($array['offset'])), 1680785659);
            }
            $offset = (int)$array['offset'];
            unset($array['offset']);
        }
        if ($array !== []) {
            throw new \InvalidArgumentException(sprintf('Unsupported pagination array key%s: "%s"', count($array) === 1 ? '' : 's', implode('", "', array_keys($array))), 1680785661);
        }
